...
function _s_posted_on() {
	$time_string = '<time class="c-post__date" datetime="%1$s">%2$s</time>';
	// if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
	// 	$time_string = '<time class="c-post__date" datetime="%1$s">%2$s</time><time class="c-post__date" datetime="%3$s">%4$s</time>';
	// }
...
if(!function_exists('_s_entry_cats')):
	function _s_entry_cats() {
		$cats = get_the_category();
		$cat_tag = '<a href="%1$s" rel="category tag">%2$s</a>,';
		$category_list = "";
		foreach ($cats as $cat) {
			$category_list .= sprintf($cat_tag, esc_url( get_category_link( $cat->term_id ) ), $cat->name);
		}
...
function _s_footer_cats() {
	$cats = get_the_category();
	$cat_tag = '<a href="%1$s" rel="tag">%2$s</a>';
	$category_list = "";
	foreach ($cats as $cat) {
		$category_list .= sprintf($cat_tag, esc_url( get_category_link( $cat->term_id ) ), $cat->name);
	}
